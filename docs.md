...
- 必填：站点名称、站点地址、头像地址。
- 选填：站点描述、联系邮箱、站点截图、友链页面地址、RSS 订阅地址、是否启用 RSS 抓取。
- 联系邮箱填写后用于接收审核通知和查询申请状态；邮箱为空时仍可提交申请。
- 公开申请提交后状态为 `pending`，不会直接进入公开友链列表。
- Turnstile 启用时，表单必须提交 `turnstile_token`，也可通过 `X-Turnstile-Token` 或 `CF-Turnstile-Token` 请求头传递。
...
| `email` | string | 否 | 联系邮箱；填写后可接收审核通知并用于查询申请状态 |
...
| `turnstile_token` | string | 条件 | Turnstile 启用时必填；也可以通过 `X-Turnstile-Token` 或 `CF-Turnstile-Token` 请求头传递 |
...
| `email` | string | 否 | 联系邮箱；如果原友链已登记邮箱，填写值必须与原记录一致 |
| `description` | string | 否 | 新站点描述 |
| `snapshot` | string | 否 | 新站点截图 URL |
| `friend_link_page` | string | 否 | 新友链页面地址 |
| `feed` | string | 否 | 新 RSS 订阅地址 |
| `enable_rss` | bool | 否 | 是否启用 RSS 抓取，默认 `false` |
| `turnstile_token` | string | 条件 | Turnstile 启用时必填；也可以通过 `X-Turnstile-Token` 或 `CF-Turnstile-Token` 请求头传递 |
